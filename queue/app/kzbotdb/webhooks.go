...
func (d *Db) GetWebhooksByEventTypeAndCorpAfter(corp string, eventType string, afterTs int64) ([]models.Webhook, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	query := `
		...
		WHERE corp = $1
		  AND tsunix > $2
		  AND message::text LIKE $3
	`
	searchPattern := fmt.Sprintf("%%%q%%", eventType) // "%\"RedStarEnded\"%"

	rows, err := d.db.Query(ctx, query, corp, afterTs, searchPattern)
	if err != nil {
		return nil, fmt.Errorf("query error: %w", err)
	}
	defer rows.Close()
