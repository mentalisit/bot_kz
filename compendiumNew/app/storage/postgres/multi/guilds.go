...
func (d *Db) GuildGet(guildId string) (*models.MultiAccountGuild, error) {
	ctx, cancel := d.getContext()
	defer cancel()
...
func (d *Db) GuildGetUUID(guildUUId uuid.UUID) (*models.MultiAccountGuild, error) {
	ctx, cancel := d.getContext()
	defer cancel()
	var u models.MultiAccountGuild
	selectGuild := "SELECT * FROM compendium.guilds WHERE gid = $1"
	err := d.db.QueryRow(ctx, selectGuild, guildUUId).Scan(&u.GId, &u.GuildName, &u.Channels, &u.AvatarUrl)
	if err != nil {
		return nil, err
	}
	return &u, nil
