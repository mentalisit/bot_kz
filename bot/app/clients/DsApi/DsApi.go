...
func (d *DsApi) CleanChat(channel string, mesid string, mtext string) {
	panic("implement me")
}

func (d *DsApi) CleanRsBotOtherMessage() {
	panic("implement me")
}

func NewTgApi(log *logger.Logger) *DsApi {
	return &DsApi{log: log, ChanRsMessage: make(chan models.InMessage, 20)}
}
...
func convertAndSend(m any) (a answer, err error) {
	...
	req, err := http.NewRequestWithContext(ctx, "POST", "http://discord/func", bytes.NewBuffer(data))
	if err != nil {
		return
	}
