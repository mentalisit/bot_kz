...
func (b *Bot) Plus(in models.InMessage) bool {
	...
	countName := b.storage.Count.CountNameQueueCorp(ctx, in.Username, in.Config.CorpName)
	message := ""
	ins := false
	if countName > 0 && in.Option.Reaction {
		b.iftipdelete(in)
		t := b.storage.Timers.UpdateMitutsQueue(ctx, in.Username, in.Config.CorpName)
		if t.Timedown > 3 {
...
func (b *Bot) Minus(in models.InMessage) bool {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 4*time.Second)
	defer cancel()
	countNames := b.storage.Count.CountNameQueueCorp(ctx, in.Username, in.Config.CorpName)
	if countNames > 0 && in.Option.Reaction {
		b.iftipdelete(in)
		t := b.storage.Timers.UpdateMitutsQueue(ctx, in.Username, in.Config.CorpName)
		if t.Name == in.Username && t.Timedown > 3 {
			message := fmt.Sprintf("%s %s%s %s %d%s",
				t.Mention, b.getText(in, "info_cannot_click_minus"), t.Lvlkz, b.getText(in, "you_will_still"), t.Timedown, b.getText(in, "min"))
			b.ifTipSendTextDelSecond(in, message, 10)
		} else if t.Name == in.Username && t.Timedown <= 3 {
			in.Lvlkz = t.Lvlkz
			bb = true
			in.Option.Reaction = false
