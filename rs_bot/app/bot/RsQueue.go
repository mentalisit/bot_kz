...
func (b *Bot) QueueLevel(in models.InMessage) {
	...
	count, err := b.storage.Count.CountQueue(in.RsTypeLevel, in.Config.CorpName)
	if err != nil {
		b.log.ErrorErr(err)
		return
	}

	numberLvl, err2 := b.storage.DbFunc.NumberQueueLvl(in.RsTypeLevel, in.Config.CorpName)
	if err2 != nil {
		b.log.ErrorErr(err)
		return
	}
	...
	if count == 0 {
		if !in.Opt.Contains(models.OptionQueue) {
			text := b.getText(in, "rs_queue") + in.RsTypeLevel + b.getText(in, "empty")
	...
	if in.Config.TgChannel != "" {
		...
		if darkStar {
			...
			ntg["text1"] = fmt.Sprintf("%s%s (%d)\n", b.getText(in, "rs_queue"), level, numberLvl)
		}
	...
	ft := func(in models.InMessage) {
		if in.Opt.Contains(models.OptionEdit) {
			err = b.client.Tg.EditMessageTextKey(in.Config.TgChannel, u.User1.Tgmesid, texttg, level)
			if err != nil {
		...
		if !in.Opt.Contains(models.OptionEdit) {
			mesidTg := b.client.Tg.SendEmbed(level, in.Config.TgChannel, texttg)
			if mesidTg != 0 {
				if err = b.storage.Update.MesidTgUpdate(mesidTg, in.RsTypeLevel, in.Config.CorpName); err != nil {
					b.log.ErrorErr(err)
				}
	...
	if len(lvlk) > 0 {
		for _, corp := range lvlk {
			if corp != "" {
				...
				if !in.Opt.Contains(models.OptionQueueAll) {
