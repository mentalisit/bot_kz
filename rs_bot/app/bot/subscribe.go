...
func (b *Bot) SubscribePing(in models.InMessage) {
	tt := fmt.Sprintf("MENTION: UserId %s RsTypeLevel %s", in.UserId, in.RsTypeLevel)
	flag := b.client.Tg.SendChannelDelSecondRsMention(in.Config.TgChannel, tt, 1800)
	if flag {
		return
	}
...
func (b *Bot) Subscribe(in models.InMessage) {
	...
	codes := func(subscribeCode int) (text string) {
		if subscribeCode == 0 {
	...
	if in.Tip == ds {
		...
		b.client.Ds.SendChannelDelSecond(in.Config.DsChannel, codes(subscribeCode), 10)
	} else if in.Tip == tg {
		...
		//}
		subscribeCode := b.client.Tg.Subscribe(in.UserId, in.RsTypeLevel, in.Config.TgChannel)
		go b.client.Tg.SendChannelDelSecond(in.Config.TgChannel, codes(subscribeCode), 10)
	}
...
func (b *Bot) Unsubscribe(in models.InMessage) {
	...
	codes := func(unsubscribeCode int) (text string) {
	...
	if in.Tip == ds {
		...
		b.client.Ds.SendChannelDelSecond(in.Config.DsChannel, codes(unsubscribeCode), 10)
	} else if in.Tip == tg {
		//suds := models.Subscribe{
		//	Name:    in.Username,
		//	Lvlkz:   in.RsTypeLevel,
		...
		//	b.storage.Subscribe.Subscribe(suds)
		//}
		unsubscribeCode := b.client.Tg.Unsubscribe(in.UserId, in.RsTypeLevel, in.Config.TgChannel)
		b.client.Tg.SendChannelDelSecond(in.Config.TgChannel, codes(unsubscribeCode), 10)
	}
}
