...
func (d *Db) EmojiModuleReadUsers(name, tip string) models.EmodjiUser {
	...
	results, err := d.db.Query(selec, name, tip)
	defer results.Close()
	if err != nil {
		d.log.ErrorErr(err)
	}
...
func (d *Db) EmojiUpdate(name, tip, slot, emo string) string {
	...
	_, err := d.db.Exec(sqlUpd, emo, name, tip)
	if err != nil {
		d.log.ErrorErr(err)
	}
...
func (d *Db) ModuleUpdate(name, tip, slot, moduleAndLevel string) string {
	...
	_, err := d.db.Exec(sqlUpd, moduleAndLevel, name, tip)
	if err != nil {
		d.log.ErrorErr(err)
	}
...
func (d *Db) WeaponUpdate(name, tip, weapon string) string {
	...
	_, err := d.db.Exec(sqlUpd, weapon, name, tip)
	if err != nil {
		d.log.ErrorErr(err)
	}
	close(ch)
