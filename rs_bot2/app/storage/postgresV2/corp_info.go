...
// ReadCorpInfoByCorpID читает запись о корпорации по corp_id
func (d *Db) ReadCorpInfoByCorpName(corpName string) (*models.CorpInfo, error) {

	query := `SELECT id, corp_name, corp_id, level, xp, webhook, last_win, date_ended, last_update 
			  FROM ws.corps_info WHERE corp_name = $1`

	var corp models.CorpInfo
	err := d.db.QueryRow(query, corpName).Scan(
		&corp.ID,
		&corp.CorpName,
		&corp.CorpID,
		&corp.Level,
...
// DeleteCorpInfo удаляет запись о корпорации по ID
func (d *Db) DeleteCorpInfo(id int64) error {

	query := `DELETE FROM ws.corps_info WHERE id = $1`
