...
func (c *Client) GetChatsMember(userId int64, full bool) []models.ChatAccess {
	req := GetChatsMemberReq{
		Userid: userId,
		Full:   full,
	}
	response, err := c.client.GetChatsMember(context.Background(), &req)
	if err != nil {
		c.log.ErrorErr(err)
		return nil
	}
	var chats []models.ChatAccess
	for _, chat := range response.Chats {
		chats = append(chats, models.ChatAccess{
