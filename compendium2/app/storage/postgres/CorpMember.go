...
func (d *Db) corpMemberInsert(ctx context.Context, guildid string, u models.CorpMember) {
	...
	err := d.db.QueryRow(ctx, "SELECT guildid FROM compendium.corpmember WHERE userid = $1 AND guildid = $2", u.UserId, guildid).Scan(&existingGuildID)
	if err != nil {
		switch {
...
func (d *Db) CorpMemberTechUpdate(ctx context.Context, userid, guildid string, tech models.TechLevels) {
	Tech, err := json.Marshal(tech)
	if err != nil {
		d.log.Info(err.Error())
	}
	sqlUpd := `update compendium.corpmember set tech = $1 where userid = $2 AND guildid = $3`
	_, err = d.db.Exec(ctx, sqlUpd, Tech, userid, guildid)
	if err != nil {
		d.log.ErrorErr(err)
	}
}
